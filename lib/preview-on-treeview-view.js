...
export default class PreviewOnTreeviewView {
    ...
    //initialize capturing of treeview events
    initCapture() {
        if (this.treeview) {
            //get tree view items
            ...
            //observe expand/collapse events
            //but treeview not provide these event(as far as I looked).
            //so I observe the changes of DOM object.
            //probably this operation is slow...
            this.observer = new MutationObserver( (MutationRecords, MutationObserver) => {
                for(let rootinfo of this.rootdirinfo){
                    //check whether the height of the element is changed
                    //to reduce CPU load.
                    const elem = rootinfo[0];
                    const oldheight = rootinfo[1];
                    if(elem.clientHeight != oldheight){
                        //changed
                        this.onUpdateTreeView();
                        //update the height
                        rootinfo[1] = elem.clientHeight;
                    }
                }
            });
            //pick up root directory element
            const rootelems = document.querySelectorAll('.tree-view-root > .project-root');
            this.rootdirinfo = [];
            if(rootelems){
                for (let i = 0; i < rootelems.length; i++) {
                    const rootinfo = [rootelems[i], rootelems[i].clientHeight];
                    this.rootdirinfo.push(rootinfo);
                    this.observer.observe(rootelems[i], { attributes: true, subtree:true });
                }
            }
        }
    }
